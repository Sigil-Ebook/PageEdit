...
# This script is for building the PageEdit appimage
# Please run this script in docker image: ubuntu:22.04
# E.g: docker run --rm -v `git rev-parse --show-toplevel`:/reporoot ubuntu:22.04 /reporoot/.github/workflows/build_pageedit_appimage.sh <short_sha or tag>
# If you need keep store build cache in docker volume, just like:
#   $ docker volume create appimage-tools
#   $ docker run --rm -v `git rev-parse --show-toplevel`:/reporoot -v appimage-tools:/var/cache/apt -v appimage-tools:/usr/src ubuntu:22.04 /reporoot/.github/workflows/build_pageedit_appimage.sh <short_sha or tag>
...
setup_python() {
  ...
  if [ -f "/reporoot/sigilpython${PYTHON_VER}.tar.xz" ]; then
    ...
    tar -xJf "/reporoot/sigilpython${PYTHON_VER}.tar.xz" -C /opt/pageedittools
  else
    ...
    tar -xJf "/usr/src/sigilpython${PYTHON_VER}.tar.xz" -C /opt/pageedittools
  fi
  export PATH=/opt/pageedittools/python/bin:$PATH
  export LD_LIBRARY_PATH=/opt/pageedittools/python/lib:$LD_LIBRARY_PATH
  export PYTHONHOME=/opt/pageedittools/python
  which python3
  echo "Upgrading pip..."
...
setup_qt6() {
  ...
  python3 -m aqt install-qt --outputdir /opt/pageedittools/Qt linux desktop "${QT6_VER_FULL}" linux_gcc_64 -m qtpositioning qtpdf qtwebchannel qtserialport qtimageformats
  export PATH=/opt/pageedittools/Qt/$QT6_VER_FULL/gcc_64/bin:$PATH
  export LD_LIBRARY_PATH=/opt/pageedittools/Qt/${QT6_VER_FULL}/gcc_64/lib:$LD_LIBRARY_PATH
...
setup_webengine() {
  ...
  if [ -f "/reporoot/AppImageWebEngine${QT6_FN}.tar.xz" ]; then
    ...
    tar -xJf "/reporoot/AppImageWebEngine${QT6_FN}.tar.xz" -C /opt/pageedittools
  else
    ...
    tar -xJf "/usr/src/AppImageWebEngine${QT6_FN}.tar.xz" -C /opt/pageedittools
  fi
  ls -l "/opt/pageedittools/Qt/${QT6_VER_FULL}/gcc_64/libexec"
  export Qt6_Dir="/opt/pageedittools/Qt/${QT6_VER_FULL}/gcc_64/lib/cmake/Qt6"
  export Qt6_DIR="/opt/pageedittools/Qt/${QT6_VER_FULL}/gcc_64/lib/cmake/Qt6"
  export QT_PLUGIN_PATH="/opt/pageedittools/Qt/${QT6_VER_FULL}/gcc_64/plugins"
}
...
build_pageedit() {
  ...
  cmake /reporoot \
    -G "Ninja" \
    -DCMAKE_PREFIX_PATH="/opt/pageedittools/Qt/${QT6_VER_FULL}/gcc_64/lib/cmake" \
    -DAPPIMAGE_BUILD=1 \
...
build_appimage() {
  export LINUXDEPLOY_OUTPUT_APP_NAME="PageEdit-${COMMITISH}"
  export APPIMAGE_EXTRACT_AND_RUN=1
  cd /build
  DEPLOY_PLATFORM_THEMES=1 \
  DISABLE_COPYRIGHT_FILES_DEPLOYMENT=1 \
  LD_LIBRARY_PATH=lib:pageedit.AppDir/usr/lib/python$PY_SHORT_VER/site-packages/pillow.libs:$LD_LIBRARY_PATH \
  ...
  python3 "${SELF_DIR}/appimg_cleanup.py" /build/pageedit.AppDir/usr/lib $PY_SHORT_VER
  cp -fv "${SELF_DIR}/AppRun" /build/pageedit.AppDir/
  ...
  cp -fv PageEdit-*-x86_64.AppImage* /reporoot/
}
